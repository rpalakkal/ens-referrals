...
impl AxiomComputeFn for ENSReferralInput {
    fn compute(
        ...
        for i in 0..N {
            ...
            let now = api.from_hi_lo(now_256);

            let duration = gate.sub(api.ctx(), expires, now);
            let (_, referrer_id_from_duration) =
                api.range.div_mod(api.ctx(), duration, 86400u64, 64);

            if i == 0 {
                referrer_id = referrer_id_from_duration;
            } else {
                api.ctx()
                    .constrain_equal(&referrer_id_from_duration, &referrer_id);
            }
